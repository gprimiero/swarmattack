...
[
  ...
  propositionHistory ;; LIST of LIST, recording the value of information received for any P at any timestamp (if anything is received at all)
  acceptanceProbability ;; float, probability that an agent will accept a piece of information - the initial value is set by the slider initialAcceptanceProbability
]
...
to setup
 ...
 ; graph-edges
  ...
  if (_is-debug = true) [ discovery 0 1 ]
end
...
to initializeTurtle [_size _color _prop#]
  ...
  set color _color
  set size _size
  set acceptanceProbability initialAcceptanceProbability
  set ranking -1
  set propositions []
  set apperception []
  set sourceTurtles []
  ...
  [
    ...
    set propositionHistory lput [] propositionHistory
...
to buildNetwork [_turtles# _networkType]
  ...
  if (_networkType = "swarm") [ buildSwarmNetwork _turtles# ]
end
...
to go
  ...
  let propIndex 0
  let network "swarm"
  ;or "linear" or "random" or "small-world" or "swarm"
  ;; if no stop
  ifelse (goCondition propIndex = true) [ goStep propIndex network ]
  ;; if time to stop
  [
...
to goStep [ _propositionIndex _networkType ]
  ;; print progress if debug
  if (_is-debug = true) [ print word "Running timestep N " ticks ]
  ;; update history for any turtle
  ask turtles [
    ;; get the history for this proposition
    let currentHistory item _propositionIndex propositionHistory
    ;; add a 0 to the history
    set currentHistory lput 0 currentHistory
    ;; replace proposition history
    set propositionHistory replace-item _propositionIndex propositionHistory currentHistory
  ]
  ;; run knowledge dynamics
  spreadKnowledge _propositionIndex _networkType
  ;; advance tick
  tick
  ;; update plots
  updateRuntimePlots
...
;to-report goCondition [ _propositionIndex ]
  ;; just for now, stop when every turtle has a truth val for the proposition or when it has been running too much
 ; report count (turtles with [ item _propositionIndex propositions = 0 ]) > 0 and ticks < 1000
;end

to-report goCondition [ _propositionIndex ]
  ;; just for now, stop when every turtle has a truth val for the proposition or when it has been running too much
  report (count (turtles with [ item _propositionIndex propositions = 1 ]) < _nodes ) or
  ( count (turtles with [ item _propositionIndex propositions = -1 ]) < _nodes ) and ticks < 1000
end


to run-sim [ _propositionIndex _sim# _networkType ]
  setup
  let counter 0
  while [ counter < _sim#]
  [
    ...
    let is_attack false
    clearP
    discovery _propositionIndex discoverer_turtles
    ...
    [
      ...
      if (proportionKnowlegde >= network_coverage_for_attack and is_attack = false)
      [
      ...
      ;; spread knowledge
      goStep _propositionIndex _networkType
    ]
    ;; finally export data
    export-data
    ;; update counter
...
to discovery [ _propositionIndex _#turtles ]
  ask n-of _#turtles turtles
  [ set breed discoverers
    knowProposition _propositionIndex -1 1 ]
end


to spreadKnowledge [ _propositionIndex _networkType ]
  ifelse ( _networkType = "swarm" )
  [
    ask turtles [ move ]
   ;; ATTENTION: NEEDED TO CHANGE THIS AS OTHERWISE NO CONFLICT WILL HAPPEN IN LEARNING SINCE ONLY NEUTRAL
   ;; TURTLE WILL ASK FOR INFO
   ;; ask turtles with [ item _propositionIndex propositions = 0 ] [ learn _propositionIndex ]
    ask turtles [ learn _propositionIndex ]
  ]
  [
...
to learn [ _propositionIndex ]
  ;; check if there are other turtles here that know something
  if count(other turtles-here with [ item _propositionIndex propositions != 0 ]) > 0
  [
    ;; pick randomly ONE of the turtle HERE that has a truth value for P
    let source [who] of one-of other turtles-here with [ item _propositionIndex propositions != 0 ]
    ;; get the truth value of the source
    let sourceTruthVal item 0 [item _propositionIndex propositions] of turtles with [who = source]
    ;; get to know that P from the source
    knowProposition _propositionIndex source sourceTruthVal
  ]
end

to attack [ _propositionIndex ]
  ;; select attackers among turtles that knows that P
  ...
  [
    ...
    ifelse (_DoS = true)
    [
      ;; back to neutral on P
      knowProposition _propositionIndex -2 0
    ]
    [
      ;; spread info that -P
...
to knowProposition [ _propositionIndex _source _truthValue ]
  ...
  [
    ...
    set finalTruthValue 0
    ifelse (_network_type = "swarm")
    [ set finalTruthValue verifySwarmPropositionProbability currentTruthValue _truthValue _propositionIndex breed]
  ...
  if (finalTruthValue = 0) [ set color blue ]
end

to-report verifySwarmPropositionProbability [ _currentVal _newVal _propositionIndex _breed ]
  ;;;;; now, reason by case
  ;;;;;

  ;; if it is an attacker, it won't change its mind
  if (_breed = attackers) [ report _currentVal ]

  ;; if it is a discoverer, it won't change its mind
  if (_breed = discoverers) [ report _currentVal ]

  if ( _currentVal = 0) [ report _newVal ]

  ;; generate random number to decide if turtle will accept the new val
  let currentRandom random-float 1
  let _finalVal 0
  ;; compare number with internal acceptance value
  ifelse (currentRandom  <= acceptanceProbability)
  [
    set _finalVal _newVal
  ]
  [
    set _finalVal _currentVal
  ]

  ;; now update acceptanceProbability
  ;; if the turtle gets same info that the one he has, P=P * K
  ifelse (_currentVal = _newVal)
  [
    set acceptanceProbability acceptanceProbability * K
  ]
  ;; ELSE, P=P+ZP if the info is different
  [
    let newP acceptanceProbability + (Z * acceptanceProbability)
    set acceptanceProbability min list 1.0 newP ;; make sure result does not exceed 1.0
  ]
...
to-report verifySwarmProposition [ _currentVal _newVal _propositionIndex _breed ]
  ;;;;; now, reason by case
  ;;;;;
  ...
  if (_currentVal = _newVal) [ report _newVal ]


  ;; if the turtle has no prior info on P, start to believe that P
  if (_currentVal = 0) or
  ;[ report _newVal ]
  ...
  (_currentVal != _newVal)
  [
    ;; update item N (N = ticks) recording the info just received
    ;; in tuna time window, condo e volte in chi sono contraddetto e se maggiore di N, cambio idea
    let currentHistory item _propositionIndex propositionHistory
    set currentHistory replace-item ticks currentHistory _newVal
    set propositionHistory replace-item _propositionIndex propositionHistory currentHistory
    ;; now count how many times in the last N ticks the _newVal item is found
    let newValCount countTruthValueInTimeWindow currentHistory DegreeConfirmation _newVal ;;
    ;; decide to change opinion on P or not based on a threshol
    ifelse (newValCount > DegreeSkepticism)
    [
      ;; print (word newValCount _newVal currentHistory)
      report _newVal
    ]
...
to-report verifyProposition [ _currentVal _newVal _propositionIndex _breed _currentWho _sourceWho]
  ...
  [
    ...
    [
        let consensus neighborConsensus _propositionIndex _newVal sourceApperception
...
to-report countTruthValueInTimeWindow [ history timeWindow truthVal ]
  let counter 0
  let truthCounter 0
  let totalTicks length history
  ;; calculate the first tick in the history as the current length of history - time window
  let startTick totalTicks - timeWindow
  ;; if the history is shorter than win, just set start to 0
  if (startTick < 0) [ set startTick 0 ]
  foreach history
  [
    if (counter >= startTick)
    [
      if (item counter history = truthVal)
      [
        set truthCounter truthCounter + 1
      ]
    ]
    ;; increment counter
    set counter counter + 1
...
to-report neighborConsensus [ _propositionIndex _truthVal _sourceApperception ]
  let total_neighbors_with_opinion count link-neighbors with [item _propositionIndex propositions != 0 and item _propositionIndex apperception > _sourceApperception]
  let total_neighbors_with_same_opinion count link-neighbors with [item _propositionIndex propositions = _truthVal and item _propositionIndex apperception > _sourceApperception]
  ...
  ;; if they all are the same, it will return true, otherwise false
...
to buildRandomNewtork [_turtles#]
  ...
  ;; as a refinement, set ranking proportional to link-neigh instead of random
  ;; ask turtles [ set ranking 1.0 / count link-neighbors ]
  ;; pretty print the layout
...
;  file-print composeLineForFile "network type" field_delimiter _network_type
  ...
  file-print _temp
  set _temp "No. discoverers"
  set _temp word _temp "\t"
  set _temp word _temp discoverer_turtles
  file-print _temp
  set _temp "No. attackers"
  set _temp word _temp "\t"
  set _temp word _temp number_attacker
  file-print _temp
  set _temp "network type"
  set _temp word _temp "\t"
  set _temp word _temp _network_type
  file-print _temp
  set _temp "Degree Skepticism"
  set _temp word _temp "\t"
  set _temp word _temp DegreeSkepticism
  file-print _temp
  set _temp "Degree Confirmation"
  set _temp word _temp "\t"
  set _temp word _temp DegreeConfirmation
  file-print _temp
  ;;Empty line
  file-print ""
  ;;Data
  ...
  set _temp "final knowers"
  set _temp word _temp "\t"
  set _temp word _temp count turtles with [ item 0 propositions = 1 ]
  file-print _temp
  set _temp "final infected"
  set _temp word _temp "\t"
  set _temp word _temp count turtles with [ item 0 propositions = -1 ]
  file-print _temp
  set _temp "final undecided"
  set _temp word _temp "\t"
  set _temp word _temp count turtles with [ item 0 propositions = 0 ]
  file-print _temp
  ...
  set _temp "K"
  set _temp word _temp "\t"
  set _temp word _temp K
  file-print _temp
  set _temp "Z"
  set _temp word _temp "\t"
  set _temp word _temp Z
  file-print _temp
  set _temp "initialAcceptanceProbability"
  set _temp word _temp "\t"
  set _temp word _temp initialAcceptanceProbability
  file-print _temp
  file-close
end
...
_network_type
_network_type
"total" "random" "small-world" "linear" "swarm"
4
...
349
Undecided agents
count turtles with [ item 0 propositions = 0 ]
17
1
11
...
406
Agents who know 1
count turtles with [ item 0 propositions = 1 ]
17
1
11
...
248
count % DoS damage
(count turtles with [ item 0 propositions = 0 ]) * 1.0 /\n(count turtles) * 1.0
17
1
11
...
contradictory-discovery</setup>
  ...
  <experiment name="5thconfig_TOTAL_100nodes_1%attackers_90%discoverers_05rankp_08coverage" repetitions="1" runMetricsEveryStep="true">
    <go>run-sim 0 1</go>
    <timeLimit steps="1000"/>
    ...
    <enumeratedValueSet variable="discoverer_turtles">
      <value value="90"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="experiment1_prob_nonadaptive_initial1.0" repetitions="1" runMetricsEveryStep="true">
    <go>run-sim 0 1 "swarm"</go>
    <timeLimit steps="1000"/>
    <metric>count turtles</metric>
    <metric>count turtles with [ item 0 propositions = 1 ]</metric>
    <metric>count turtles with [ item 0 propositions = -1 ]</metric>
    <metric>count turtles with [ item 0 propositions = 0 ]</metric>
    <enumeratedValueSet variable="DegreeSkepticism">
      <value value="3"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="rankP">
      <value value="1"/>
    ...
    <enumeratedValueSet variable="DegreeConfirmation">
      <value value="20"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="_DoS">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="network_coverage_for_attack">
      <value value="0.1"/>
    ...
    <enumeratedValueSet variable="initialAcceptanceProbability">
      <value value="1"/>
    </enumeratedValueSet>
  </experiment>
</experiments>
